...
import { Home, Briefcase, Users, DollarSign, MessageSquare, LifeBuoy, Globe } from "lucide-react";
...
const dashboardNav = (userType) => [
  { label: "Dashboard", icon: <Home />, path: `/${userType === "vendor" ? "vendor-dashboard" : "customer"}` },
  { label: "Jobs", icon: <Briefcase />, path: `/${userType === "vendor" ? "vendor-dashboard" : "customer"}/jobs` },
  { label: "Messages", icon: <MessageSquare />, path: `/${userType === "vendor" ? "vendor-dashboard" : "customer"}/messages` },
  { label: userType === "vendor" ? "Earnings" : "Payments", icon: <DollarSign />, path: `/${userType === "vendor" ? "vendor-dashboard/earnings" : "customer/payments"}` },
  { label: "Support", icon: <LifeBuoy />, path: `/${userType === "vendor" ? "vendor-dashboard" : "customer"}/support` },
];
    ...
    <>
      ...
      >
        <div className="flex items-center justify-center h-16 border-b border-border">
          <span className="text-2xl font-bold text-primary">BOJJ {userType === "vendor" ? "Vendor" : "Customer"}</span>
          <button
            type="button"
              ...
              return (
                <li key={item.label}>
                  <Button
                    ...
                    className={`w-full justify-start gap-3 px-4 py-2 rounded-none transition group ${isActive ? "!bg-secondary text-secondary-foreground" : ""}`}
                  >
                    <Link
                      to={item.path}
                      onClick={onClose}
                    ...
                    >
                      {item.icon}
                      <span className="flex-1 text-left group-hover:text-primary transition-colors">{item.label}</span>
                    </Link>
                  </Button>
                </li>
              );
...
const DashboardLayout = ({ children, userType }: DashboardLayoutProps) => {
  ...
  const businessName = userType === "vendor" ? `${firstName}'s Construction!` : "";

  return (
    <SidebarProvider>
      <div className="flex min-h-screen bg-background text-foreground">
        {/* Sidebar: drawer on mobile, sticky on desktop */}
        <Sidebar userType={userType} isOpen={sidebarOpen} onClose={() => setSidebarOpen(false)} setSidebarOpen={setSidebarOpen} />
        {/* Main Content */}
