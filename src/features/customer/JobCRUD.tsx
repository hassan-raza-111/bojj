    ...
    <div className="w-full">
      <div className="flex justify-between items-center mb-6 mt-10">
        ...
        <Link to="/customer/jobs/new">
          <Button className="bg-white text-bojj-primary border border-bojj-primary hover:bg-bojj-primary hover:text-white transition-colors flex items-center gap-2">
        ...
        <Table>
          ...
          <TableBody>
            {filteredJobs.length === 0 ? (
              <TableRow>
                <TableCell colSpan={9} className="text-center text-gray-500 py-8">No jobs found.</TableCell>
              </TableRow>
            ) : (
              filteredJobs.map((job) => (
                <TableRow key={job.id}>
                  ...
                  <TableCell>
                    <span className={`px-2 py-1 rounded text-xs font-semibold ${job.status === "completed" ? "bg-blue-100 text-blue-700" : "bg-green-100 text-green-700"}`}>
                      {job.status === "completed" ? "Completed" : "Active"}
                    </span>
                  </TableCell>
                  <TableCell>{job.status === "completed" ? job.completedDate : job.postedDate}</TableCell>
                  ...
                  <TableCell>
                    <div className="flex gap-2">
                      ...
                      <Button size="icon" variant="ghost" onClick={() => handleDelete(job.id)} title="Delete">
