...
import { Search, Check, ChevronLeft, ChevronRight, Star, ChevronDown } from "lucide-react";
...
const HomePage = () => {
  ...
  const [carouselApi, setCarouselApi] = useState<CarouselApi | null>(null);
    ...
    useEffect(() => {
      ...
      setAuthStatus({ isAuthenticated: auth, userRole: role });
    }, []);
    
    const isVendor = authStatus.userRole === "vendor";
    const isCustomer = authStatus.userRole === "customer";
  ...
  const handleSearch = (e: React.FormEvent) => {
    ...
    toast({
      ...
      description: `Searching for ${service} services in ${location || "your area"}`,
    });
  };
  ...
  const testimonials = [
    {
      quote: "Found an amazing contractor within hours. The whole process was seamless.",
      name: "Sarah Johnson",
      role: "Customer",
      rating: 5,
    ...
    {
      quote: "Bojj has transformed my business. I'm getting new clients every week now.",
    ...
    {
      quote: "I needed a plumber urgently and found one within an hour. Amazing service!",
      name: "Emma Rodriguez",
      role: "Customer",
      rating: 5,
    ...
    {
      quote: "As a small business owner, Bojj has helped me reach more clients than I ever could before.",
      name: "David Chen",
      role: "Vendor",
      rating: 4,
    ...
    {
      quote: "The quality of professionals on this platform is outstanding. Very satisfied!",
    ...
    {
      quote: "I've been able to grow my landscaping company by 40% since joining Bojj last year.",
      name: "James Wilson",
      role: "Vendor",
      rating: 5,
  ...
  const renderStars = (rating: number) => {
    return Array(5).fill(0).map((_, index) => (
      <Star 
        key={index}
        size={16} 
        className={index < rating ? "fill-bojj-primary text-bojj-primary" : "text-gray-300"} 
      />
    ));
  };
          ...
          <div className="flex flex-col-reverse lg:flex-row gap-8 items-center">
            ...
            <div className="w-full lg:w-1/2">
              ...
              <p className="text-base md:text-lg text-gray-600 dark:text-gray-300 mb-8 text-center lg:text-left">
                Tell us what you need, set your budget, and get competitive bids from trusted vendors in your area.
              </p>
              ...
              <form onSubmit={handleFormSubmit} className="flex flex-col gap-4">
                ...
                <div className="relative">
                  ...
                  >
                    <span className={`${service ? "text-gray-900 dark:text-white" : "text-gray-500 dark:text-gray-400"}`}>
                    ...
                    <div className="absolute z-50 w-full mt-1 bg-white dark:bg-gray-800 rounded-md shadow-lg border border-gray-200 dark:border-gray-700">
                      {Object.entries(serviceCategories).map(([category, services]) => (
                        ...
                        >
                          ...
                          <div className={`absolute left-full top-0 w-64 bg-white dark:bg-gray-800 rounded-md shadow-lg border border-gray-200 dark:border-gray-700
          ...
          >
            <CarouselContent className="-ml-2 md:-ml-4">
              {testimonials.map((testimonial, i) => (
                <CarouselItem key={i} className="pl-2 md:pl-4 sm:basis-1/1 md:basis-1/2 lg:basis-1/3">
                  <div
                    className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-sm border border-gray-100 dark:border-gray-700 h-full"
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-bojj-primary dark:text-bojj-primary/90">{testimonial.role}</p>
                      </div>
                    </div>
                    ...
                    <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">{testimonial.location}</p>
                  </div>
                </CarouselItem>
              ))}
            </CarouselContent>
            ...
            <div className="flex justify-center gap-1 mt-4">
              {testimonials.slice(0, Math.min(6, Math.ceil(testimonials.length / 3))).map((_, i) => (
                <div
                  key={i}
                  className={`h-2 w-2 rounded-full transition-colors duration-300 ${
                    Math.floor(currentCarouselSlide / 3) === i ? "bg-bojj-primary" : "bg-gray-300"
                  }`}
                />
              ))}
            </div>
  ...
  <div className="container mx-auto px-4 md:px-6 text-center">
    ...
    <Link to="/signup?type=customer">
      ...
      <AuthPromptModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} />
